...
class SenderState(rx.State):
    ...
    # Certificate selection specific to sender
    selected_sender_cert: str = ""
    ...
    async def set_sender_and_determine_recipient(self, sender: str):
        ...
        self.selected_sender_cert = sender  # Track the selected certificate
        ...
        try:
            ...
            self.key_path = (
                self.cert_path.replace(".crt", ".key") if self.cert_path else ""
            )
    ...
    async def send_file(self):
        ...
        if (
            not self.sender_entity
            or not self.recipient_entity
            or not self.selected_file_name
        ):
        ...
        try:
            ...
            print(f"Endpoint UUID: {endpoint_uuid}, Participant ID: {participant_id}")
                ...
                rx.vstack(
                    rx.hstack(
                        ...
                        rx.hstack(
                            ...
                            rx.link(
                                rx.button(
                                    rx.icon("download", size=18, margin_right="8px"),
                                    "Switch to Receiver",
                                    variant="outline",
                                    color_scheme="gray",
                                    size="2",
                ...
                rx.cond(
                    ...
                    rx.box(
                        rx.hstack(
                            rx.icon(
                                rx.cond(
                                    SenderState.status_is_error,
                                    "alert_circle",
                                    "check_circle_2",
                                ),
                                size=20,
                                color=rx.cond(
                                    SenderState.status_is_error, "red.500", "green.500"
                                ),
                            ),
                            rx.text(
                                ...
                                font_weight="medium",
                                color=rx.cond(
                                    SenderState.status_is_error, "red.700", "green.700"
                                ),
                            ),
                            rx.spacer(),
                            rx.icon(
                        ...
                        padding="4",
                        bg=rx.cond(SenderState.status_is_error, "red.50", "green.50"),
                        border="1px solid",
                        border_color=rx.cond(
                            SenderState.status_is_error, "red.200", "green.200"
                        ),
                        border_radius="lg",
                        margin_bottom="6",
                ...
                rx.box(
                    rx.vstack(
                        rx.hstack(
                            ...
                            rx.heading("Entity Selection", size="6", color="gray.700"),
                            align_items="center",
                            spacing="2",
                        ),
                    ...
                    rx.box(
                        rx.vstack(
                            rx.hstack(
                                ...
                                rx.heading(
                                    "Endpoint Details", size="6", color="gray.700"
                                ),
                                align_items="center",
                                spacing="2",
                            ),
                ...
                rx.box(
                    rx.vstack(
                        rx.hstack(
                            ...
                            rx.heading("File Selection", size="6", color="gray.700"),
                            align_items="center",
                            spacing="2",
                        ),
                        rx.divider(color="gray.200"),
                        rx.vstack(
                            ...
                            rx.box(
                                rx.input(
                                    type="file",
                                    on_change=lambda e: SenderState.handle_file_select(
                                        e
                                    ),
                                    accept=".csv,.pdf,.txt,.dat,.xlsx,.docx,.zip",
                                    width="100%",
                                ),
                                border="2px dashed",
                            ...
                            rx.hstack(
                                rx.cond(
                                    ...
                                    rx.hstack(
                                        rx.icon(
                                            "check_circle", size=16, color="green.500"
                                        ),
                                        rx.text(
                                    ...
                                    rx.hstack(
                                        rx.icon("file", size=16, color="gray.400"),
                                        rx.text("No file selected", color="gray.500"),
                                        align_items="center",
                                        spacing="2",
                                    ),
                                ),
                ...
                rx.box(
                    rx.vstack(
                        rx.hstack(
                            ...
                            rx.heading("Recent Transfers", size="6", color="gray.700"),
                            align_items="center",
                            spacing="2",
                        ),
                        rx.divider(color="gray.200"),
                            ...
                            rx.table.root(
                                ...
                                rx.table.body(
                                    rx.table.row(
                                        rx.table.cell(
                                            rx.hstack(
                                                ...
                                                rx.text(
                                                    "example.pdf", font_weight="medium"
                                                ),
                                                align_items="center",
                                                spacing="2",
                                            )
                                        ...
                                        rx.table.cell(
                                            rx.text("Entity A", color="gray.600")
                                        ),
                                        rx.table.cell(
                                            rx.text("Entity B", color="gray.600")
                                        ),
                                        rx.table.cell(
                                            rx.badge(
                                                "Completed",
                                    ...
                                    rx.table.row(
                                        ...
                                        rx.table.cell(
                                            rx.text("Entity C", color="gray.600")
                                        ),
                                        rx.table.cell(
                                            rx.badge(
                                                "Pending",
