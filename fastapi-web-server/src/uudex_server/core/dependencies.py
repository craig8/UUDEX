...
async def get_current_user(request: Request, db: Annotated[AsyncSession,
                                                           Depends(get_db)]) -> AuthenticatedUser:
    """
    ...
    if not cert_cn:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED,
                            detail="Unauthorized: No certificate found")
    ...
    if not endpoint:
        raise HTTPException(status_code=HTTP_401_UNAUTHORIZED,
                            detail="Unauthorized: Invalid certificate")

    return AuthenticatedUser(endpoint=endpoint)


async def get_admin_user(
        current_user: Annotated[AuthenticatedUser,
                                Depends(get_current_user)]) -> AuthenticatedUser:
    """
...
async def get_participant_admin_user(
    ...
    if not current_user.is_participant_admin():
        raise HTTPException(status_code=HTTP_403_FORBIDDEN,
                            detail="Participant administrator access required")
    return current_user
