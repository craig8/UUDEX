...
async def get_subject_by_id(subject_id: int, session: Annotated[AsyncSession,
                                                                Depends(get_db)]) -> Subject:
...
async def create_dataset(
        dataset: DatasetCreate, session: Annotated[AsyncSession, Depends(get_db)],
        user: Annotated[AuthenticatedUser, Depends(get_current_user)]) -> DatasetRead:
    """
    ...
    return DatasetRead(dataset_id=created_dataset.dataset_id,
...
async def get_all_datasets(
        ...
        user: Annotated[AuthenticatedUser, Depends(get_current_user)]) -> list[DatasetRead]:
    """
    ...
    repo = DatasetRepository(session)
    datasets = await repo.select_datasets_by_participant(session=session,
                                                         participant_id=participant_id)

    # Convert to DatasetRead (metadata only, no payload)
    return [
        DatasetRead(dataset_id=dataset.dataset_id,
                    ...
                    create_datetime=dataset.create_datetime) for dataset in datasets
    ]


@dataset_router.get("/{dataset_id}")
async def get_dataset_by_id(
        dataset_id: int, session: Annotated[AsyncSession, Depends(get_db)],
        user: Annotated[AuthenticatedUser, Depends(get_current_user)]) -> DatasetRead:
    """
    ...
    # Return DatasetRead (metadata only, no payload)
    return DatasetRead(dataset_id=dataset.dataset_id,
                       dataset_uuid=dataset.dataset_uuid,
                       dataset_name=dataset.dataset_name,
                       description=dataset.description,
                       properties=dataset.properties,
...
async def download_dataset(
        dataset_id: int, session: Annotated[AsyncSession, Depends(get_db)],
        user: Annotated[AuthenticatedUser, Depends(get_current_user)]) -> Response:
    """
    ...
    return Response(content=dataset.payload,
                    media_type="application/octet-stream",
                    headers={
                        "Content-Disposition":
                        f"attachment; filename=dataset_{dataset_id}_payload",
