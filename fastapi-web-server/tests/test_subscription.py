...
@pytest.mark.asyncio
async def test_create_subscription(subscription_repo: r.SubscriptionRepository,
...
@pytest.mark.asyncio
async def test_select_subscription_by_id(subscription_repo: r.SubscriptionRepository,
                                         participant_repo: r.ParticipantRepository):
    created_participant = await setup_participant(participant_repo=participant_repo)
    subscription = m.SubscriptionCreate(subscription_uuid="uuid1",
                                        subscription_name="name1",
...
@pytest.mark.asyncio
async def test_update_subscription(subscription_repo: r.SubscriptionRepository,
                                   participant_repo: r.ParticipantRepository):
    created_participant = await setup_participant(participant_repo=participant_repo)
    subscription = m.SubscriptionCreate(subscription_uuid="uuid1",
                                        subscription_name="name1",
...
@pytest.mark.asyncio
async def test_delete_subscription(subscription_repo: r.SubscriptionRepository,
                                   participant_repo: r.ParticipantRepository):
    created_participant = await setup_participant(participant_repo=participant_repo)
    subscription = m.SubscriptionCreate(subscription_uuid="uuid1",
                                        subscription_name="name1",
...
@pytest.mark.asyncio
async def test_select_user_subscriptions(authenticated_user: m.AuthenticatedUser,
...
@pytest.mark.asyncio
async def test_select_admin_subscriptions(subscription_repo: r.SubscriptionRepository,
                                          participant_repo: r.ParticipantRepository):
    created_participant = await setup_participant(participant_repo=participant_repo)
    subscription = m.SubscriptionCreate(subscription_uuid="uuid1",
                                        subscription_name="name1",
...
@pytest.mark.asyncio
async def test_select_subscription_by_uuid(subscription_repo: r.SubscriptionRepository,
                                           participant_repo: r.ParticipantRepository):
    created_participant = await setup_participant(participant_repo=participant_repo)
    subscription = m.SubscriptionCreate(subscription_uuid="uuid1",
                                        subscription_name="name1",
